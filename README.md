...
-- NO TYPOS | ALL EXECUTORS COMPATIBLE
-- ==============================================
...
local SAVE_KEY_USED = "BlueMode_UsedTime_v3"
local SAVE_KEY_COOLDOWN = "BlueMode_CooldownEnd_v3"
local SAVE_KEY_VOLUME = "BlueMode_Volume_v3"
...
local function ClearESP()
    for _, Player in pairs(Players:GetPlayers()) do
        if Player and Player.Character then
            pcall(function()
                if Player.Character:FindFirstChild("BLUE_Outline") then
                    Player.Character.BLUE_Outline:Destroy()
                end
                if Player.Character:FindFirstChild("FriendRainbowDot") then
                    Player.Character.FriendRainbowDot:Destroy()
                end
            end)
        end
    end
...
local function UpdateVolume(newVolume)
    MusicVolume = math.clamp(newVolume, 0, 1)
...
local function PlaySound(SoundID)
    ...
    CurrentSound.SoundId = FormatSoundID(SoundID)
...
VolLabelMain.AutoLocalize = false
...
VolNumTextMain.AutoLocalize = false
...
UserInputService.InputChanged:Connect(function(Input)
    if SliderActiveMain and Input.UserInputType == Enum.UserInputType.MouseMovement then
        local Pos = math.clamp((Input.Position.X - VolBGMain.AbsolutePosition.X) / VolBGMain.AbsoluteSize.X, 0, 1)
