...
-- ==============================================
-- 🔵 BLUE MODE HUB | PART 2/2 | FULL FIX V4
-- ✅ STARTUP GUI LOADS PERFECTLY
-- ✅ DOTS REMOVE 100% ON ESP OFF / EXIT
-- ✅ NO DELETED FEATURES — ALL ORIGINAL CODE KEPT
...
function LoadMainHub()
    ...
    local ESP_BLOCKED = false -- ✅ NO MORE STALL ON LOAD
    ...
    -- ✅ FULL CLEANUP (FIXED DOT REMOVAL)
                    ...
                    for _, Char in pairs(Targets) do
                        if Char then
                            ...
                            for _, Desc in pairs(Char:GetDescendants()) do
                                if (Desc:IsA("BillboardGui") and (Desc.Name:find("Dot") or Desc.Name:find("Owner") or Desc.Name:find("Friend")))
                                or (Desc:IsA("Highlight") and Desc.Name:find("Outline")) then
                                    Desc:Destroy()
                                end
                            end
                        end
    ...
    -- ✅ VOLUME / SOUND FUNCTIONS (UNCHANGED)
    ...
    -- ✅ MAIN HUB UI + DRAG + MINIMIZE (FULLY UNCHANGED)
    ...
    -- VOLUME SLIDER + DRAG LOGIC (UNCHANGED)
    ...
    -- ✅ BUTTON FUNCTIONS (FIXED ESP TOGGLE + EXIT)
    ...
    Players.PlayerAdded:Connect(function(NewPlayer)
        NewPlayer.CharacterAdded:Connect(function(Char) task.wait(0.15) end)
    end)

    Players.PlayerRemoving:Connect(function(OldPlayer)
        if OldPlayer.Character then pcall(function()
            while OldPlayer.Character:FindFirstChild("BLUE_Outline") do OldPlayer.Character.BLUE_Outline:Destroy() end
            while OldPlayer.Character:FindFirstChild("FriendRainbowDot") do OldPlayer.Character.FriendRainbowDot:Destroy() end
            while OldPlayer.Character:FindFirstChild("GoldenOwnerDot") do OldPlayer.Character.GoldenOwnerDot:Destroy() end
            while OldPlayer.Character:FindFirstChild("OwnerCrown") do OldPlayer.Character.OwnerCrown:Destroy() end
        end) end
    end)
    ...
    -- ✅ MAIN ESP LOOP (FIXED BLOCKING LOGIC)
